    ...
    )
{
	...
	//DriverObject->MajorFunction[IRP_MJ_INTERNAL_DEVICE_CONTROL] = KeyboardClassDeviceControl;
    ...
    )
{
    ...
    int                       numKeys, i;
	PKEYBOARD_INDICATOR_PARAMETERS buffer;
    //
...
NTSTATUS Ctrl2capControlComplete( 
    IN PDEVICE_OBJECT DeviceObject, 
    IN PIRP Irp,
    IN PVOID Context 
    )
{
	...
	DbgPrint(("Ctrl2capControlComplete\n" ));
    //
    // Request completed - look at the result.
    //
    IrpSp = IoGetCurrentIrpStackLocation( Irp );
...
NTSTATUS KeyboardClassDeviceControl(
...

{
    ...
    *nextIrpStack = *currentIrpStack;
    ...
    // Set the completion callback, so we can "frob" the keyboard data.
    //	    
    IoSetCompletionRoutine( Irp, Ctrl2capControlComplete, 
                            DeviceObject, TRUE, TRUE, TRUE );
